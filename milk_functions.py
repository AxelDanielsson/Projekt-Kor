...
def get_number_in_order(times):
    """
    old way to get order, not in summary csv anymore
...
def valid_start(df, tag, time, area_dict):
    """
    Check if first cow in milking session is from valid area
...
def summary_dataframe(df, area_dict):
    """
    create summary dataframe for a day.

    Parameters
    ----------
    df : pd.Dataframe
        FA data after stationary tags and one group removed.
    area_dict : dictionary
        Dictionary with boundaries for areas.

    Returns
    -------
    group_df : pd.Dataframe
        Summary dataframe for day and group of input dataframe.
...
def cum_plot(times):
    """
    cumulative plot for entry or exit 

    Parameters
    ----------
    times : dict
        entry or exit times in dict.
